...
BINS := $(addprefix bin/$(ARCH)/,kilosubspace)
PROJECT := kilosubspace
...
.container-$(ARCH)-$(VERSION): $(BINS) Dockerfile
	...
	echo $(BASE_IMAGE)@$$SHA; \
	docker build -t $(IMAGE):$(ARCH)-$(VERSION) --build-arg FROMBUILD=$(BUILD_IMAGE) --build-arg FROM=$(BASE_IMAGE)@$$SHA --build-arg GOARCH=$(ARCH) .
